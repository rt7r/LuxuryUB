...
async def startupmessage():
    """
    Start up message with Primary and Backup link support
    """
    ...
    caption = "**‏᯽︙ سـورس لـوكجـوري يـعـمـل بـنـجـاح ✓ \n᯽︙ أرسل `.الاوامر` لرؤية أوامر السورس \n ᯽︙ المطور : @rt7r_2 **"
    buttons = [(Button.url("سـورس لـوكجـوري", "https://t.me/ee2en"),)]
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        await luxur.tgbot.send_message(
            BOTLOG_CHATID,
            f'- تم بنجاح استدعاء الأوامر الإضافية \n**عدد الملفات التي استدعيت:** `{success}`\n**فشل في استدعاء :** `{", ".join(failure)}`',
        )
        ...
        try:
            ...
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
                        "᯽︙الأذونات مفقودة لإرسال رسائل لـ PRIVATE_GROUP_BOT_API_ID المحدد."
                    )
            ...
            LOGS.error(
                "᯽︙لا يمكـن العثور على فار المجموعة PRIVATE_GROUP_BOT_API_ID. تأكد من صحتها."
            )
        except Exception as e:
            LOGS.error(
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if luxuryBRANCH := branch:
        ...
        errtext = f"لا يوجد فرع باسم `{luxuryBRANCH}` في الريبو الخارجي {luxuryREPO}. تأكد من اسم الفرع عبر فار (`EXTERNAL_REPO_BRANCH`)"
    else:
        repourl = luxuryREPO
    ...
    if not os.path.exists(cfolder):
        LOGS.error(
            "هنالك خطأ أثناء استدعاء رابط الملفات الإضافية يجب التأكد من الرابط أولاً "
        )
        return await luxur.tgbot.send_message(
            BOTLOG_CHATID,
            "هنالك خطأ أثناء استدعاء رابط الملفات الإضافية يجب التأكد من الرابط أولاً ",
        )
