from LuxuryUB import bot, luxur
...
'''
mm = '''
قم بلأنضمام الى قناة لوكـجوري @ee2en
'''
keyboard = [
  [  
  ...
  [
    Button.url("سورس لوكـجوري 💎", "https://t.me/ee2en")
    ]
]
  ...
  if event.sender_id == bot.uid:
      async with bot.conversation(event.chat_id) as x:
        keyboard = [
          ...
          [
            Button.url("المطور", "https://t.me/rt7r_2")
            ]
        ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 1:
        ...
        await event.reply(i + "\n\nشكراً لأستخدامك سورس لوكـجوري ❤️. \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nشكرا لأستخدامك سورس لوكـجوري", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i == True:
      	await event.reply("لقد تم انهاء جميع الجلسات شكراً لأستخدامك لوكـجوري.", buttons=keyboard)
      else:
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("القناة", "https://t.me/ee2en")
        ]
    ]
