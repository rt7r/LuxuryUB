...
def main_menu():
    text = f"**- [سـورس لـوكجـوري](https://t.me/ee2en)\n المـساعد الـرسمي لـوكجـوري لـ {mention} 💎**"

    buttons = [
        (Button.inline("ℹ️ • التعليمات •", data="check"),),
        (
            Button.inline(f"👮‍♂️ • اوامر الادمن • ({len(GRP_INFO['admin'])})", data="admin_menu", style="danger"),
            Button.inline(f"🤖 • اوامر البوت • ({len(GRP_INFO['bot'])})", data="bot_menu", style="primary"),
        ),
        (
            Button.inline(f"🎨 • اوامر التسلية • ({len(GRP_INFO['fun'])})", data="fun_menu", style="success"),
            Button.inline(f"🎈 • اوامر الصيغ والبحث • ({len(GRP_INFO['misc'])})", data="misc_menu", style="success"),
        ),
        (
            Button.inline(f"🧰 • الادوات • ({len(GRP_INFO['tools'])})", data="tools_menu", style="primary"),
            Button.inline(f"🗂 • الملفات • ({len(GRP_INFO['utils'])})", data="utils_menu", style="primary"),
        ),
        (
            Button.inline(f"🧸 • اوامر متفرعه • ({len(GRP_INFO['extra'])})", data="extra_menu", style="primary"),
            Button.inline("🔒 • اغلاق القائمة •", data="close", style="danger"),
        ),
    ]
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("luxur"):
            ...
            builder = event.builder
            troll = os.path.join("./LuxuryUB", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس لوكـجوري 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./LuxuryUB", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس لوكـجوري 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./LuxuryUB", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس لوكـجوري 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© LuxuryUB Help",
                description="Help menu for LuxuryUB",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
...
@luxur.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@luxur.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@luxur.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@luxur.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
