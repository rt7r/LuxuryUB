...
@luxur.on(admin_cmd(pattern="كتابة(?: |$)(.*)"))
async def _(event):
...
@luxur.on(admin_cmd(pattern="صوتية(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@luxur.on(admin_cmd(pattern="فيد(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@luxur.on(admin_cmd(pattern="لعبة(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@luxur.on(admin_cmd(pattern="الرابط$"))
async def _(e):
...
@luxur.on(admin_cmd(pattern="للكل تاك$"))
async def listall(LuxuryUB):
    if LuxuryUB.fwd_from:
        return
    mentions = "- هذه هي قائمة جميع الاعضاء هنا: "
    chat = await bot.get_input_chat()
    async for x in borg.iter_participants(chat, 2000):
        mentions += f" \n[{x.first_name}](tg://user?id={x.id})"
    await LuxuryUB.reply(mentions)
    await LuxuryUB.delete()
...
@luxur.on(admin_cmd(pattern=r"سبونج"))
async def kerz(kerz):
    await kerz.edit(R)
...
@luxur.on(admin_cmd(pattern=r"كلب"))
async def dog(dog):
    await dog.edit(M)
Z = (
    "┈┏━╮╭━┓┈╭━━━━╮\n"
...
@luxur.on(admin_cmd(pattern=r"ذئب"))
async def fox(fox):
    await fox.edit(H)


@luxur.on(admin_cmd(pattern=r"فيل"))
async def elephant(elephant):
    await elephant.edit(A)


@luxur.on(admin_cmd(pattern=r"هومر"))
async def homer(homer):
    await homer.edit(N)


@luxur.on(admin_cmd(pattern=r"بك"))
async def pig(pig):
    await pig.edit(Z)
