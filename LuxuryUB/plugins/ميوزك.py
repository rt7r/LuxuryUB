...
MUSIC_CMDS = "(شغل 1|شغل فيديو|شغل|تشغيل|فيديو 1|فيديو|انضمام|مغادرة|افتح مكالمة|اطفاء مكالمة|حالة المكالمة|تخطي|وكف مؤقتا|وكف|وقف|كمل|استمرار|ايقاف نهائي|ايقاف|قائمة التشغيل)"
...
async def process_music_command(event, cmd, target_id_str, query, reply):
    ...
    if cmd in ["اوكف", "وكف", "وقف"]:
        try:
            await luxur.pytgcalls.pause_stream(event.chat_id) 
    ...
    if cmd in ["كمل", "استمرار"]:
        try:
            await luxur.pytgcalls.resume_stream(event.chat_id) 
