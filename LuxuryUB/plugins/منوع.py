...
async def fetch_prayer_times():
    ...
    times_json = await luxur.download_file(file_location)
    return times_json
...
async def send_prayer_times(event):
    ...
    input_file = await luxur.upload_file(bytes(times_json, 'utf-8'), part_size_kb=512)
    await luxur.send_file(chat_id, input_file, caption=f"وقت الفجر: {fajr_time}\nالتاريخ الهجري: {hijri_date}", force_document=True)

@luxur.on(admin_cmd(pattern="صلاه(?: |$)(.*)"))
async def handle_command(event):
    await send_prayer_times(event)

@luxur.on(admin_cmd(pattern="دعوه للمكالمه(?: |$)(.*)"))
async def _(e):
...
@luxur.on(admin_cmd(pattern="بدء مكالمه(?: |$)(.*)"))
async def _(e):
    try:
...
R = [
    ...
    "**-** مطور السورس **⪼ [𐇮 𓂐 𓆩الــسيد𓆪 𖠛🇷🇺 ](t.me/rt7r_2)   \n"
    "**-** قناة السورس **⪼ [𐇮 لوكـجوري ](t.me/ee2en)   "
]

@luxur.on(admin_cmd(pattern="بلي$"))
...
@luxur.on(admin_cmd(pattern="اذكار$"))
async def ithker(knopis):
...
@luxur.on(admin_cmd(pattern="خيروك$"))
async def ithker(knopis):
    await knopis.edit(choice(HuRe_5erok))

    
@luxur.on(admin_cmd(pattern="اذكار عشر$"))    
...
@luxur.on(admin_cmd(pattern="شنو رأيك بهذا$"))
async def ithker(knopis):
...
@luxur.on(admin_cmd(pattern="بوسة$"))
async def ithker(knopis):
...
async def handle_finance(event):
    if event.sender_id not in DevJoker:
        return
    
    message_text = event.message.text
    
    if message_text.startswith("تمويل"):
        if len(message_text.split()) > 1:
            channel_username = message_text.split()[1].replace("@", "")
            try:
                await luxur(JoinChannelRequest(channel_username))
                await event.reply("**᯽︙ تم الانضمام إلى القناة بنجاح!**")
            except Exception as e:
                await event.reply(f"**خطأ في الانضمام: {str(e)}**")
        else:
            await event.reply("**᯽︙ يُرجى تحديد معرف القناة او المجموعة مع التمويل يامطوري ❤️**")
    
    elif message_text.startswith("ارشف"):
        if len(message_text.split()) > 1:
            channel_username = message_text.split()[1].replace("@", "")
            try:
                # الانضمام أولاً
                ...
                await luxur.edit_folder(entity, folder=1)
                
                await event.reply("**᯽︙ تم الانضمام إلى القناة بنجاح ووضعها في مجلد الأرشيف!**")
            except Exception as e:
                await event.reply(f"**خطأ: {str(e)}**")
        else:
            await event.reply("**᯽︙ يُرجى تحديد معرف القناة او المجموعة مع الأرشيف يامطوري ❤️**")

client = luxur

@luxur.on(admin_cmd(pattern="فك الحظر$"))
async def handle_unblock_all(event):
    ...
    for user in blocked_users.users:
        try:
            ...
            continue
@luxur.on(admin_cmd(pattern="(تاريخه|تاريخة)$"))
...
@luxur.on(admin_cmd(pattern="بروكسي$"))
async def get_proxy(event):
    ...
    # رسالة البداية
    await event.edit("**᯽︙ جاري البحث عن أحدث بروكسي...**")
    
    # حساب البنك (Ping) بطريقة بسيطة ومستقرة
    start_time = time.time()
    await event.client.get_me()
    end_time = time.time()
    ping = round((end_time - start_time) * 1000, 2)
    
    channels = ["TelMTProto", "ProxyMTProto"]
    proxies = []
    # نمط استخراج رابط البروكسي من النص أو الأزرار
    pattern = r"https://t\.me/proxy\?server=[^&\s]+&port=[^&\s]+&secret=[^&\s]+"
    
    try:
        for channel in channels:
            # جلب آخر 10 منشورات للفحص
            result = await event.client(GetHistoryRequest(
                peer=channel,
                limit=10,
            ...
            for post in result.messages:
                # 1. فحص الروابط داخل الأزرار الشفافة (كما في الصور التي أرسلتها)
                if post.reply_markup:
                    for row in post.reply_markup.rows:
                        for button in row.buttons:
                            if hasattr(button, 'url') and button.url:
                                found_in_btn = re.findall(pattern, button.url)
                                if found_in_btn:
                                    proxies.extend(found_in_btn)
                
                # 2. فحص الروابط داخل نص المنشور
                if post.message:
                    found_in_msg = re.findall(pattern, post.message)
                    if found_in_msg:
                        proxies.extend(found_in_msg)
        
        if not proxies:
            return await event.edit("**᯽︙ لم يتم العثور على أي بروكسي متاح حالياً.**")
        
        # اختيار أحدث رابط (الأول في القائمة)
        best_proxy = proxies[0]
        
        # الرسالة النهائية: الرابط والبنك فقط وبدون مصادر
        final_msg = (
            "**᯽︙ تم جلب أحدث بروكسي بنجاح ✓**\n\n"
            f"**🔗 رابط الاتصال :** [إضغط هنا للاتصال]({best_proxy})\n\n"
            f"**📡 البنك (Ping) :** `{ping}ms`"
        )
        
        await event.edit(final_msg, link_preview=False)

    except Exception as e:
        # منع انهيار السورس عند حدوث خطأ
        await event.edit(f"**⚠️ عذراً، حدث خطأ مفاجئ:**\n`{str(e)}`")

# CODE BY Luxury | DEVELOPER: @ee2en
from telethon import events
from LuxuryUB import luxur

@luxur.ar_cmd(pattern="ايدي_رمز$")
async def get_emoji_id(event):
    # التحقق من الرد على رسالة
    reply = await event.get_reply_message()
    if not reply or not reply.entities:
        return await event.edit("**⚠️ يرجى الرد على رمز بريميوم لاستخراج الآيدي!**")

    # استخراج آيدي الرمز المخصص (Custom Emoji)
    for entity in reply.entities:
        if hasattr(entity, 'document_id'):
            emoji_id = entity.document_id
            await event.edit(f"**✅ آيدي الرمز هو:**\n\n`{emoji_id}`")
            return

    await event.edit("**❌ هذا الرمز ليس بريميوم أو مخصص!**")
