...
@luxur.ar_cmd(
    ...
    info={
        ...
        "examples": "{tr}رسالة @ee2en  حي الله  ان",
    },
)
...
@luxur.ar_cmd(pattern="ازالة التوجيه")
async def Reda (event):
    if event.message.reply_to_msg_id:
        ...
        if replied_msg.media:
            ...
            await luxur.send_message(event.chat_id, "", file=replied_msg.media)
        else:
            
            await luxur.send_message(chat_id=event.chat_id, message=replied_msg.message)
    else:
