from LuxuryUB import luxur, bot
import time
from telethon.tl import types
from LuxuryUB import BOTLOG_CHATID
    ...
    if gvarstatus("subprivate"):
        if event.is_private:
            try:
                ...
                if idd == 1165225957 and not gvarstatus("developer_aljoker"):
                    ...
                    ch = gvarstatus("pchan")
                    if not ch:
                        return await luxur.tgbot.send_message(BOTLOG_CHATID, "** انت لم تضع قناة الاشتراك الاجباري قم بوضعها**")
                    try:
                        ch = int(ch)
                    except BaseException as r:
                        return await luxur.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ \n{r}**")
                    url = f"https://api.telegram.org/bot{tok}/getchatmember?chat_id={ch}&user_id={idd}"
                    req = requests.get(url)
                    reqt = req.text
                    if "chat not found" in reqt:
                        mb = await luxur.tgbot.get_me()
                        mb = mb.username
                        await luxur.tgbot.send_message(BOTLOG_CHATID, f"**البوت الخاص بك @{mb} ليس في قناة الاشتراك الاجباري**")
                        return
                    if "bot was kicked" in reqt:
                        mb = await luxur.tgbot.get_me()
                        mb = mb.username
                        await luxur.tgbot.send_message(BOTLOG_CHATID, "** البوت الخاص بك @{mb} مطرود من قناة الاشتراك الاجباري اعد اضافته**")
                        return
                    if "not found" in reqt:
                        try:
                            c = await luxur.get_entity(ch)
                            chn = c.username
                            if c.username == None:
                                ra = await luxur.tgbot(ExportChatInviteRequest(ch))
                                chn = ra.link
