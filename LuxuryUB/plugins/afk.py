...
async def set_not_afk(event):
    ...
    if AFK_.afk_star != {}:
        ...
        endtime = ""
        if d > 0:
            endtime += f"{d} أيام {h} ساعات {m} دقائق {s} ثواني"
        elif h > 0:
            endtime += f"{h} ساعات {m} دقائق {s} ثواني"
        else:
            endtime += f"{m} دقائق {s} ثواني" if m > 0 else f"{s} ثواني"
    ...
    if (("afk" not in current_message) or ("#afk" not in current_message)) and (
        ...
        shite = await event.client.send_message(
            event.chat_id,
            "**✧ ¦ تم تعطيـل وضع السـليب والـرجوع إلى الوضع الطبيعي، أهلاً بك!**",
        )
        AFK_.USERAFK_ON = {}
        AFK_.afk_time = None
        ...
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
                "**✧ ¦ إشعار:**\n"
                + f"`تم تعطيل وضع السليب والرجوع للوضع الطبيعي. كان مفعل لمدة: {endtime}`",
            )
...
async def on_afk(event):
    ...
    if AFK_.USERAFK_ON and not (await event.get_sender()).bot:
        msg = None
        message_to_reply = f"**✧ ¦ عـذراً، أنا الآن في وضعية عدم الاتصال 💤** \n**✧ ¦ مدة الغياب 🕐:** `{endtime}`"
        ...
        resalt = f"<b>✧ ¦ تنبيه أثناء وضع السليب من مجموعة: </b><code>{hmm.title}</code>"
        if full is not None:
...
@luxur.ar_cmd(
    ...
    info={
        "header": "تفعيل وضع السليب لحسابك",
        "usage": [
            "{tr}سليب <السبب>",
        ],
    },
)
async def sleep_text(event):
    ...
    if not AFK_.USERAFK_ON:
        ...
        if AFK_.reason:
            await edit_delete(
                event, f"**✧ ¦ أنا الآن في وضعية عدم الاتصال، يُرجى المراسلة لاحقاً بسبب: {AFK_.reason} 💤**", 5
            )
        else:
            await edit_delete(event, f"**✧ ¦ أنا الآن في وضعية عدم الاتصال، يُرجى المراسلة لاحقاً 💤**", 5)
            
        if BOTLOG:
            if AFK_.reason:
                await event.client.send_message(
                    BOTLOG_CHATID,
                    f"**✧ ¦ أمـر السليـب 💤:** \n **تم تشغيل الوضع مع ذكر السبب:** {AFK_.reason}",
                )
            else:
                await event.client.send_message(
                    BOTLOG_CHATID,
                    f"**✧ ¦ أمـر السليـب 💤:** \n **تم تشغيل الوضع بدون ذكر سبب ❕**",
                )
...
@luxur.ar_cmd(
    ...
    info={
        "header": "تفعيل وضع السليب لحسابك مع الوسائط",
        "usage": [
            "{tr}سليب_ميديا <السبب> بالرد على وسائط",
        ],
    },
)
async def sleep_media(event):
    ...
    if media_t == "Sticker" or not media_t:
        return await edit_or_reply(
            event, "**✧ ¦ أمـر السـليب ميديا: المرجـو قم بالـرد علـى صورة أو فيديو مع الأمر**"
        )
        
    if not BOTLOG:
        return await edit_or_reply(
            event, "✧ ¦ لإستخـدام هذا الأمر يجـب إضافة متغير PRIVATE_GROUP_BOT_API_ID"
        )
...
CMD_HELP.update(
    {
    "السليب": "\n**✧ ¦ أوامر الغياب (السليب):**\n\n`.سليب` <مع ذكر السبب (اختياري)>\nلتحويل حسابك إلى وضع عدم الاتصال (يرد نصياً).\n\n`.سليب_ميديا` <بالرد على صورة/فيديو>\nلتحويل حسابك لوضع عدم الاتصال مع إرسال وسائط."
    }
)