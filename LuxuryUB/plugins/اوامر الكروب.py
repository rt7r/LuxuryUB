...
async def ban_user(chat_id, i, rights):
    try:
        await luxur(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)        

@luxur.ar_cmd(pattern="ارسل")
...
@luxur.ar_cmd(pattern="حظر_الكل(?:\s|$)([\s\S]*)")
async def banall(event):
     ...
     try:
         ...
         pass
@luxur.ar_cmd(pattern="كتم_الكل(?:\s|$)([\s\S]*)")
async def muteall(event):
     ...
     try:
         ...
         pass
@luxur.ar_cmd(pattern="طرد_الكل(?:\s|$)([\s\S]*)")
async def kickall(event):
     ...
     try:
         ...
         pass
@luxur.ar_cmd(pattern="الغاء التفليش")
...
@luxur.ar_cmd(pattern="مغادرة الكروبات")
async def Reda (event):
    ...
    try:
        ...
        for group in gr:
            if group not in dd:
                await luxur.delete_dialog(group)
                num += 1
                await sleep(1)
        if num >=1:
        ...
        if channel_username:
            try:
                ...
                if isinstance(entity, Channel) and entity.creator or entity.admin_rights:
                    ...
                    await luxur(LeaveChannelRequest(channel_username))
...
@luxur.ar_cmd(pattern="تصفية الخاص")
async def hussein(event):
...
@luxur.ar_cmd(pattern="تصفية البوتات")
async def Hussein(event):
...
@luxur.ar_cmd(pattern=r"الذكاء تعطيل")
...
async def Hussein(event):
    ...
    if sender_id != 705475246:
        if isinstance(event.message.entities, list) and any(isinstance(entity, MessageEntityCustomEmoji) for entity in event.message.entities):
            ...
            aljoker_entity = await luxur.get_entity(sender.id)
...
@luxur.ar_cmd(pattern="المميز تفعيل")
...
@luxur.ar_cmd(pattern="المميز تعطيل")
async def disable_emoji_blocker(event):
    global Ya_Hussein
    Ya_Hussein = False
...
async def handle_new_message(event):
    ...
    if message_counts[user_id]['count'] >= 3:
        try:
            await luxur.edit_permissions(event.chat_id, user_id, send_messages=False)
            sender = await event.get_sender()
            aljoker_entity = await luxur.get_entity(sender.id)
            aljoker_profile = f"[{aljoker_entity.first_name}](tg://user?id={aljoker_entity.id})"
...
async def get_bot_entity():
    return await luxur.get_entity('me')

@luxur.on(events.NewMessage(outgoing=True, pattern=r'\.اسرع (.*)'))
    ...
    if is_game_started and not is_word_sent and word.lower() in event.raw_text.lower():
        if event.chat_id:
            ...
            if bot_entity and event.sender_id != bot_entity.id:
                ...
                points_text = '\n'.join([f'{i+1}• {(await luxur.get_entity(participant_id)).first_name}: {participant_points}' for i, (participant_id, participant_points) in enumerate(sorted_points)])
                await luxur.send_message(event.chat_id, f'الف مبرووووك 🎉 اللاعب ( {sender_first_name} ) فاز! \n اصبحت نقاطة: {points[winner_id]}\nنقاط المشاركين:\n{points_text}')
joker = [
    "تلعب وخوش تلعب 👏🏻",
    "لك عاش يابطل استمر 💪🏻",
...
@luxur.on(events.NewMessage(pattern=r'\طك (\d+)'))
...
@luxur.on(events.NewMessage(pattern=r'\جيب (\d+)'))
    ...
    if chat_id in group_game_status and group_game_status[chat_id]['is_game_started2'] and event.sender_id == group_game_status[chat_id]['joker_player']:
        ...
        if 1 <= guess <= 6:
            if guess == correct_answer:
                ...
                points_text = '\n'.join([f'{i+1}• {(await luxur.get_entity(participant_id)).first_name}: {participant_points}' for i, (participant_id, participant_points) in enumerate(sorted_points)])
                game_board = [["💍" if i == correct_answer - 1 else "🖐️" for i in range(6)]]
                await luxur.send_message(event.chat_id, f'الف مبروووك 🎉 اللاعب ( {sender_first_name} ) وجد المحبس 💍!\n{format_board(game_board, numbers_board)}')
                game_board = [row[:] for row in original_game_board]
                await luxur.send_message(event.chat_id, f'نقاط اللاعب : {points[winner_id]}\nنقاط المشاركين:\n{points_text}')
            else:
                game_board = [["💍" if i == correct_answer - 1 else "🖐️" for i in range(6)]]
...
@luxur.on(events.NewMessage(pattern=r'\انا'))
