...
@l313l.ar_cmd(pattern=r"حفظ_المحتوى (.+)", command=("حفظ_المحتوى", plugin_category))
async def save_media(event):
    ...
    try:
        message = await l313l.get_messages(channel_id, ids=int(message_id))
        if not message or not (message.media or message.document):
            return await event.edit("**᯽︙ الرسالة لا تحتوي على ميديا قابلة للحفظ ⚠️**")
        
        file_path = await l313l.download_media(message, file=temp_dir)
        await l313l.send_file('me', file=file_path, caption=message.text or "")
        os.remove(file_path)
        await event.edit(f"**᯽︙ تم حفظ الميديا بنجاح، اذهب إلى (الرسائل المحفوظة) ✓**")
    except Exception as e:
        await event.edit(f"**᯽︙ حدث خطأ أثناء الحفظ:** `{str(e)}`")

@l313l.ar_cmd(pattern="تحويل صورة$", command=("تحويل صورة", plugin_category))
...
@l313l.ar_cmd(pattern="تحويل ملصق$", command=("تحويل ملصق", plugin_category))
...
@l313l.ar_cmd(pattern=r"تحويل متحركة ?([0-9.]+)?$", command=("تحويل متحركة", plugin_category))
...
@l313l.ar_cmd(pattern="تحويل (mp3|بصمة|voice)$", command=("تحويل", plugin_category))
...
@l313l.ar_cmd(pattern="الغاء سيف$", command=("الغاء سيف", plugin_category))
...
@l313l.ar_cmd(pattern="سيف(?: |$)(.*) (\d+)", command=("سيف", plugin_category))
...
@l313l.ar_cmd(pattern=r"بنتيرست (.+)", command=("بنتيرست", plugin_category))
