...
# --- نظام العزل والقاعدة ---
def get_db():
    db_path = f"vars_{l313l.uid}.json"
...
# --- محرك الوقتي ---
async def time_loop():
    while not l313l.is_connected(): await asyncio.sleep(1)
    while True:
        db = get_db()
        if not db: 
        ...
        if db.get("autoname") == "true":
            try:
                ...
                await l313l(functions.account.UpdateProfileRequest(first_name=first_n, last_name=last_n))
            except: pass
        ...
        if db.get("autobio") == "true":
            try:
                ...
                await l313l(functions.account.UpdateProfileRequest(about=bio[:70]))
            except: pass
        ...
        if db.get("digitalpic") == "true":
            try:
                ...
                p_path = f"pfp_{l313l.uid}.png"
                if not os.path.exists(p_path):
                    SmartDL(img_url, p_path, progress_bar=False).start(blocking=True)
                img = Image.open(p_path)
                draw = ImageDraw.Draw(img)
                fnt = ImageFont.truetype("JoKeRUB/helpers/styles/PaybAck.ttf", 65)
                draw.text((200, 200), hm, font=fnt, fill="white")
                img.save("upload.png")
                await l313l(functions.photos.UploadProfilePhotoRequest(file=await l313l.upload_file("upload.png")))
            except: pass
        ...
        if db.get("digitalgroup"):
            try:
                ...
                await l313l(functions.channels.EditTitleRequest(channel=chid, title=f"{db.get('DEFAULT_GROUP', 'Group')} {z_hm}"))
            except: pass

        await asyncio.sleep(60)

# --- أوامر التفعيل ---
@l313l.ar_cmd(pattern="(اسم وقتي|بايو وقتي|الصورة الوقتية)$")
...
@l313l.ar_cmd(pattern="انهاء (اسم وقتي|بايو وقتي|الصورة الوقتية)$")
...
@l313l.ar_cmd(pattern="كروب (وقتي|صورة وقتي)$")
...
@l313l.ar_cmd(pattern="انهاء كروب وقتي$")
...
l313l.loop.create_task(time_loop())